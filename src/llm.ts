...
import { SummaryTopic, UserStats, UserTitle, GoldenQuote, TokenUsage } from './types'
...
export class LLMService extends Service {
  ...
  private async _callLLM<T>(prompt: string, taskName: string): Promise<{ result: T[], tokenUsage: TokenUsage }> {
  ...
  public async summarizeTopics(messagesText: string): Promise<{ result: SummaryTopic[], tokenUsage: TokenUsage }> {
  ...
  public async analyzeUserTitles(users: UserStats[]): Promise<{ result: UserTitle[], tokenUsage: TokenUsage }> {
    ...
    const prompt = this.config.promptUserTitles.replace('{users}', userSummaries);
    return this._callLLM(prompt, '用户称号分析');
  }

  public async analyzeGoldenQuotes(messagesText: string, maxQuotes: number): Promise<{ result: GoldenQuote[], tokenUsage: TokenUsage }> {
