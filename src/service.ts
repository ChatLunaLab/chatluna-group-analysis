...
export class AnalysisService extends Service {
    ...
    public async getGroupHistory(
        ...
        while (messages.length < maxMessages && queryRounds < maxRounds) {
            try {
                ...
                const result: any = await (bot as any).internal._get(
                ...
                const oldestMsg: OneBotMessage =
                    roundMessages[roundMessages.length - 1]
                queryRounds++ // 只有在成功获取后才增加轮数

                logger.info(
                    `群 ${guildId} [第 ${queryRounds} 轮] 获取了 ${roundMessages.length} 条消息。最旧消息: ${new Date(oldestMsg.time * 1000).toLocaleString()}`
                )

                messages.push(
                    ...roundMessages.map((msg: OneBotMessage) => ({
                        ...msg,
                ...
                await new Promise((res) =>
                    setTimeout(res, this.config.apiCallDelay || 800)
                ) // 避免请求过快
            } catch (err) {
                logger.error(
                    `群 ${guildId} 获取消息失败（第 ${queryRounds + 1} 轮）:`,
                    err
                )
        ...
        logger.info(
            `成功从 API 获取到 ${messages.length} 条原始消息，过滤后剩余 ${finalMessages.length} 条。`
        )
        ...
        try {
            ...
            if (messages.length < this.config.minMessages) {
                await this.ctx.bots
                    ...
                    ?.sendMessage(
                        guildId,
                        `❌ 消息数量不足（${messages.length}条），至少需要 ${this.config.minMessages} 条消息才能进行有效分析。`
                    )
                return
            }
            ...
            if (format === 'image') {
                ...
                const pdfBuffer =
                    await this.ctx.renderer.renderPdf(analysisResult)
                await bot.sendMessage(
                    guildId,
                    pdfBuffer
                ...
                await bot.sendMessage(
                    guildId,
                    this.generateTextReport(analysisResult)
                )
            }
        } catch (error) {
            ...
            await this.ctx.bots
                ...
                ?.sendMessage(
                    guildId,
                    `❌ 分析失败: ${errorMessage}。请检查网络连接和LLM配置，或联系管理员。`
                )
        }
    }
    ...
    public async executeAutoAnalysisForEnabledGroups() {
        ...
        const enabledGroups = await this.ctx.database.get(
    ...
    private _calculateBasicStats(messages: OneBotMessage[]) {
        ...
        for (const msg of messages) {
            ...
            stat.lastActive = new Date(
                Math.max(stat.lastActive.getTime(), msg.time * 1000)
            )
            ...
            for (const el of elements) {
                if (el.type === 'text') {
                    ...
                    // OneBot v11 may use this for stickers
            ...
            if (pureText) {
                allMessagesText.push(
                    `${msg.sender.nickname}: ${pureText.trim()}`
                )
            }
        }
        ...
        for (const userId in userStats) {
            ...
            stat.avgChars =
                stat.messageCount > 0
                    ? parseFloat(
                          (stat.charCount / stat.messageCount).toFixed(1)
                      )
                    : 0
            stat.nightRatio =
                stat.messageCount > 0
                    ? parseFloat(
                          (stat.nightMessages / stat.messageCount).toFixed(2)
                      )
                    : 0
            stat.replyRatio =
                stat.messageCount > 0
                    ? parseFloat(
                          (stat.replyCount / stat.messageCount).toFixed(2)
                      )
                    : 0
        }
    ...
    public async analyzeMessages(
        ...
        const { userStats, totalChars, totalEmojiCount, allMessagesText } =
            this._calculateBasicStats(messages)
        ...
        const [
            ...
            this.ctx.llm.analyzeGoldenQuotes(
                messagesText,
